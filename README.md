...
面向公路骑行组织者和骑友的核心 MVP 开发切片，覆盖活动发现、创建与发布、报名与取消、路书展示以及我的活动。项目包含可运行的微信原生小程序、NestJS API、PostgreSQL/PostGIS 数据层和高保真 Web 演示。

## 工程目录

- `apps/mini-program`：原生微信小程序 + TypeScript，支持 Mock 演示和真实 API 两种模式。
- `apps/api`：NestJS 11 + Fastify 模块化单体，包含 JWT/微信登录、活动、路书和报名模块。
...
本次交付是核心开发切片，不等同于生产发布完成。活动编辑/取消通知、GPX 导入清洗、内容审核、运营后台、共享限流和生产运维仍在后续范围；PostgreSQL 真库集成测试及 iOS/Android 微信真机验收也必须在发布前完成。具体阻断项见 `tests/qa-review-report.md`。
