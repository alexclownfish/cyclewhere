...
function loadEvents() { return wx.getStorageSync(EVENT_KEY) || clone(mock_data_1.events); }
function saveEvents(items) { wx.setStorageSync(EVENT_KEY, items); }
function loadRegistrations() { return wx.getStorageSync(REGISTRATION_KEY) || clone(mock_data_1.initialRegistrations); }
function saveRegistrations(items) { wx.setStorageSync(REGISTRATION_KEY, items); }
const delay = (value) => new Promise((resolve) => setTimeout(() => resolve(clone(value)), 180));
exports.mockApi = {
    async listEvents() { return delay(loadEvents()); },
    async getEvent(id) {
    ...
    async getEventParticipants(eventId) {
        ...
        if (hasActiveRegistration && !event.ownedByMe)
            participants.push({
    ...
    async getEventParticipantContact() {
        const profile = wx.getStorageSync('demo_profile') || {};
    ...
    async listRoutes() { return delay(mock_data_1.routes); },
    async getRoute(id) {
        const item = mock_data_1.routes.find((route) => route.id === id);
        if (!item)
            throw new Error('路书不存在');
        return delay(item);
    },
    async getMyRegistrations() { return delay(loadRegistrations()); },
    ...
    async register(eventId, input) {
        ...
        if (!event || !(0, domain_1.canRegister)(event))
            throw new Error('活动名额已满或不可报名');
        const registration = {
    ...
    async cancelRegistration(eventId) {
        ...
        if (event) {
            ...
            if (event.status === 'full' && event.registeredCount < event.capacity)
    ...
    async cancelEvent(eventId) {
        const allEvents = loadEvents();
        const event = allEvents.find((item) => item.id === eventId);
        if (!event || !event.ownedByMe)
            throw new Error('仅活动组织者可以取消活动');
        if (event.status !== 'published' && event.status !== 'full')
            throw new Error('只有已发布的活动可以取消');
    ...
    async updateProfile(profile) {
        const result = { id: (wx.getStorageSync('demo_account') || {}).id || 'demo-user', ...profile };
